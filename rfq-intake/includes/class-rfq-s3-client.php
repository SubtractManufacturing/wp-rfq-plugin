...
class RFQ_S3_Client implements RFQ_S3_Client_Interface {
    ...
    public function head_object( string $key ): array|false|WP_Error {
        try {
            ...
            return [
                ...
                'content_type'   => is_string( $content_type ) && $content_type !== '' ? $content_type : null,
                'last_modified'  => $last_modified instanceof \DateTimeInterface
        ...
        try {
            ...
            do {
                ...
                foreach ( $result['Contents'] ?? [] as $object ) {
                    ...
                    if ( ! is_string( $key ) || ! preg_match( '#^intake/([a-f0-9-]{36})/#', $key, $matches ) ) {
                        continue;
                    }

                    $session_id    = $matches[1];
                    $last_modified = $object['LastModified'] ?? null;
                    $timestamp     = $last_modified instanceof \DateTimeInterface
                        ? $last_modified->getTimestamp()
                        : 0;

                    if ( ! isset( $sessions[ $session_id ] ) || $timestamp > $sessions[ $session_id ]['last_modified'] ) {
                        $sessions[ $session_id ] = [
                ...
                $continuation = ( $result['IsTruncated'] ?? false )
    ...
    public function create_presigned_put(
        ...
        try {
            ...
            $command = $this->aws_client->getCommand(
                'PutObject',
                [
					'Bucket'      => $this->bucket,
					'Key'         => $key,
