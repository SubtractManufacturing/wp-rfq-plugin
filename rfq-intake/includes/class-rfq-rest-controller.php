...
class RFQ_REST_Controller
{
    ...
    public static function upload_urls(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        if (! self::session_has_required_contact($session)) {
            return new WP_Error(
                ...
                __('Contact information is required before upload URLs can be requested.', 'rfq-intake'),
                ['status' => 409]
            );
        }
    ...
    private static function get_session_row(string $session_id): ?array
    {
        ...
        $row = $wpdb->get_row(
            $wpdb->prepare(
                'SELECT session_id, status, contact_first_name, contact_last_name, contact_email FROM ' . $wpdb->prefix . 'rfq_sessions WHERE session_id = %s',
                $session_id
            ),
            ARRAY_A
        );
    ...
    private static function session_has_required_contact(array $session): bool
    {
        foreach (['contact_first_name', 'contact_last_name', 'contact_email'] as $field) {
            if (! isset($session[$field]) || ! is_string($session[$field]) || trim($session[$field]) === '') {
                return false;
            }
        }

        return filter_var($session['contact_email'], FILTER_VALIDATE_EMAIL) !== false;
    }
