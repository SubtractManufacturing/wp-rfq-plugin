...
**When adding or changing a QA script** (`scripts/qa-*.sh`, `scripts/test-*.sh`, `scripts/smoke-*.sh`) or an `npm run` / `composer` test command: register it in `qa-reference.md` in the same PR. That file is the living index agents use for pre-merge and JM-Sandcastle-PR-Fixes runs.
