...
const PART_MAX_BYTES = 500 * 1024 * 1024;
...
export function StepUploads({ fetchImpl = fetch }: { fetchImpl?: typeof fetch }) {
  ...
  const uploadPartFile = async (part: PartRow, file: File) => {
    ...
    updatePart({ ...part, partFile: pending });
    ...
    try {
      ...
      await uploadFile(response.upload_url, file, "application/octet-stream", () => undefined, fetchImpl);
      updatePart({
        ...part,
        partFile: { ...pending, file_key: response.file_key, status: "confirmed", progress: 100 },
      });
    } catch {
      updatePart({ ...part, partFile: failedFile(file, "Upload failed.") });
    }
  };
        ...
        <div className="rounded-lg border border-slate-200 p-4" key={part.part_id}>
          ...
          {part.partFile?.status === "error" ? (
            <div className="mt-2 text-sm text-red-700">
              <p>{part.partFile.error ?? "Upload failed."}</p>
              <button
          ...
          {part.partFile?.status === "confirmed" ? <p className="mt-2 text-sm text-green-700">Uploaded</p> : null}
        </div>
      ))}
      <button
